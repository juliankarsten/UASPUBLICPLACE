...
export default class Boiler extends Component {
  ...
  pressRow(item) {        
    ...
    firebase.database().ref('halte/'+item).on('value', (snap) => {
      this.setState({
  ...
  deleteRow(item) {
    firebase.database().ref('halte').child(item).remove();
                    ...
                    <TouchableHighlight
                      ...
                      onPress = {() => {
                        if(this.state.nama == "" || this.state.alamat == "" || this.state.jamKerja == "") {
                          ...
                          firebase.database().ref('halte').child(this.state.setItem).update({
...
AppRegistry.registerComponent("listhalteadmin", () => listhalteadmin);